...
namespace AutoClicker.Native
{
    ...
    internal static class NativeMethods
    {
        ...
        // ─────────────────────────────────────────────────────────────────────
        //  Background clicking (post messages to a window without moving the real
        //  cursor — the "second mouse" / don't-hijack-my-mouse feature)
        // ─────────────────────────────────────────────────────────────────────
        ...
        [DllImport("user32.dll", CharSet = CharSet.Unicode)]
        public static extern int GetClassName(IntPtr hWnd, System.Text.StringBuilder text, int maxCount);

        // (WM_* mouse-message constants and GetWindowThreadProcessId already exist
        // elsewhere in this file; reused by InputSimulator.BackgroundClick.)

        // Identity of the window a background click is aimed at. WindowFromPoint returns
        // whatever child happens to be under the point, and that changes as an app
        // re-lays-out its own controls — useless for answering "is this still the same
        // app I aimed at?". GA_ROOT walks up to the top-level window, which is stable.
        [DllImport("user32.dll")]
        public static extern IntPtr GetAncestor(IntPtr hWnd, uint gaFlags);

        /// <summary>GetAncestor: retrieve the root (top-level) window.</summary>
        ...
        // wParam virtual-key flags for the button messages.
        ...
        // Foreground-window tracking, used to keep macro keystrokes aimed at the window
        // the user triggered playback over (typically the game) rather than losing them
        // to Tempo / the desktop after Tempo minimises itself for the run.
        [DllImport("user32.dll")]
        ...
        // Deliver the event as a hardware SCAN CODE (wScan) rather than a virtual key
        // (wVk). Games that read the keyboard through DirectInput / Raw Input see scan
        // codes only, so WASD / arrow-key movement is ignored unless sent this way.
        public const uint KEYEVENTF_SCANCODE = 0x0008;

        // MapVirtualKey translation types
        public const uint MAPVK_VK_TO_VSC = 0x00; // virtual key -> scan code
        ...
        // Low-level keyboard hook flag: event was injected by SendInput / keybd_event.
