...
namespace AutoClicker.Native
{
    ...
    public sealed class MouseHookEventArgs : EventArgs
    {
        ...
        /// <summary>True if this event was synthesised (SendInput/mouse_event) rather than
        /// a real physical move/click — the recorder skips these so it never records
        /// Tempo's own clicking/playback.</summary>
        public bool Injected { get; }

        /// <summary>
        /// Set by a handler to SWALLOW this event so it never reaches the window
        /// beneath (the hook then returns 1 instead of calling the next hook). Used by
        /// the second cursor to keep a right-click on the marker from also opening the
        /// underlying window's context menu, and a place-click from also clicking it.
        /// Handlers that don't set it (e.g. the recorder) are unaffected.
        /// </summary>
        public bool Handled { get; set; }

        public MouseHookEventArgs(MouseHookEventType type, int x, int y, int wheelDelta, uint timeStamp, bool injected = false)
        {
            Type = type;
            X = x;
            Y = y;
    ...
    public sealed class LowLevelMouseHook : IDisposable
    {
        ...
        public bool Start()
        {
            ...
            if (_hookHandle == IntPtr.Zero)
            {
                ...
                Logger.Error($"[Hooks] failed to install the mouse hook. Win32 error {error}.");
                return false;
            }

            Logger.Info("[Hooks] low-level mouse hook installed.");
            return true;
        }
        ...
        public void Stop()
        {
            if (_hookHandle != IntPtr.Zero)
            {
                ...
                Logger.Info("[Hooks] low-level mouse hook removed.");
            }
        }
        ...
        private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error("[Hooks] exception inside the mouse hook callback.", ex);
                }
            }
