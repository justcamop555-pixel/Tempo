...
namespace AutoClicker.UI
{
    ...
    public sealed class MacroEditorForm : Form
    {
        ...
        /// <summary>
        /// Themes this dialog's scroll bars — the step list scrolls, and dialogs were
        /// never covered by the main form's scroll-bar theming pass.
        /// </summary>
        ...
        private readonly LiveStepListView _list;
        ...
        public MacroEditorForm(Theme theme, Macro macro)
        {
            ...
            Text = Utils.Localization.F("Edit Macro – {0}", _working.Name);
            ...
            // Same colour language as the Macros tab's Live Monitor — a step is the same
            // colour whether you are watching it play or editing it.
            _list = new LiveStepListView
            {
                Left = 12,
                Top = 12,
                Width = 420,
            ...
            y += 12;
            Add("Insert script…", (s, e) => InsertScript());
            ...
            // UiFactory already translated the caption; re-assigning it here put the raw
            // English back, which is why this button stayed "Save" in every language.
        ...
        private void RefreshList()
        {
            ...
            _summary.Text = Localization.F("{0} steps\n≈ {1} ms total",
                _working.StepCount, _working.EstimatedDurationMs);
        }
        ...
        private static string DetailFor(MacroAction a)
        {
            switch (a.Type)
            {
                ...
                case MacroActionType.Script:
                    return a.ScriptFileName() + "  ·  " + a.ScriptTimeoutMs + " ms";
                default:
                    return $"({a.X}, {a.Y})";
            }
        }
        ...
        /// <summary>
        /// Inserts a Script step — or edits the selected one, so double work isn't needed
        /// to change a timeout. No surrounding delay is added, unlike a key: the script's
        /// own run time IS the gap.
        /// </summary>
        private void InsertScript()
        {
            ...
            {
                int index = FirstSelectedIndex();
                MacroAction selected = index >= 0 && index < _working.Actions.Count
                    ? _working.Actions[index] : null;
                bool editing = selected != null && selected.Type == MacroActionType.Script;

                using (var dlg = new ScriptStepForm(_theme, editing ? selected : null))
                {
                    if (dlg.ShowDialog(this) != DialogResult.OK || dlg.Result == null) { return; }

                    if (editing)
                    {
                        _working.Actions[index] = dlg.Result;
                    }
                    else
                    {
                        _working.Actions.Insert(InsertIndex(), dlg.Result);
                    }
                    RefreshList();
                }
            }
            catch (Exception ex) { Utils.Logger.Swallow("MacroEditorForm.InsertScript", ex); }
        }
