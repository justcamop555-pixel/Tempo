    ...
    public sealed class TimerResolution : IDisposable
    {
        ...
        public TimerResolution(uint periodMs = 1)
        {
            ...
            try
            {
                ...
                if (_active)
                {
                    Logger.Info($"Raised system timer resolution to {_period} ms.");
                }
                else
                {
                    Logger.Warn("Could not raise the system timer resolution; using the default.");
                }
            }
            catch (Exception ex)
            {
        ...
        public void Dispose()
        {
            ...
            if (_active)
            {
                try
                {
                    ...
                    Logger.Info("Restored the system timer resolution.");
                }
                catch (Exception ex)
                {
